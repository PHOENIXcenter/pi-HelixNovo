...
class PeakEncoder(MassEncoder):
    ...
    def forward(self, X, precursors):
        ...
        encoded[:,(encoded.shape[1]//2):,:]= - encoded[:,(encoded.shape[1]//2):,:]
        intensity = self.int_encoder(X[:, :, [1]])
        #m_over_z_prime=-(X[:,:,0]-precursors[:,[0]])
        #m_over_z_prime=m_over_z_prime.unsqueeze(2)
        #encoded1 = super().forward(m_over_z_prime)
        
        
        #if self.dim_intensity is None:
            #return encoded + intensity + encoded1 
        return encoded + intensity
        #return torch.cat([encoded + intensity, encoded1 + intensity], dim=2)
